...
class VectorN(object):
    ...
    def cross(self, Vector_B):
        if isinstance(Vector_B,VectorN) and len(Vector_B) == 3:
            crossed_vector = VectorN((0, 0, 0))
            crossed_vector[0] = self[1] * Vector_B[2] - self[2] * Vector_B[1]
            crossed_vector[1] = self[2] * Vector_B[0] - self[0] * Vector_B[2]
            crossed_vector[2] = self[0] * Vector_B[1] - self[1] * Vector_B[0]
            return crossed_vector
        else:
            raise TypeError("Crossed Vectors must be of the same length!")

    def pairwise(self, Vector_B):
        if isinstance(Vector_B, VectorN) and len(Vector_B) == len(self):
            pairwise_vector = VectorN(len(self))
            for i in range(len(self)):
                pairwise_vector[i] = self[i] * Vector_B[i]
            return pairwise_vector
        else:TypeError("must be two VectorN's of equal size")